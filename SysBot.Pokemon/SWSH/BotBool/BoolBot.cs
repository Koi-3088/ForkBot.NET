...
public sealed class BoolBotSWSH : EncounterBotSWSH
{
    ...
    {
        var enumVal = Enum.GetNames<LairSpeciesBlock>();
    ...
    private async Task Skipper(long origPosix, CancellationToken token)
    {
        ...
        while (!token.IsCancellationRequested)
        {
            ...
            if (currentTime.Date.Year >= 2060)
            {
                ...
                posix = new DateTimeOffset(newTime).ToUnixTimeSeconds();
                if (!await Connection.SetSwitchTime(posix, 0_500, token).ConfigureAwait(false))
                {
            ...
            do
            {
                ...
                species = (DexRecSpecies)BitConverter.ToUInt16([.. currentspecies.Take(2)], 0);
                if (species != 0)
                    log += $"\n - {species}";
