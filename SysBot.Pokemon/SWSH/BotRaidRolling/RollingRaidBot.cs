...
public class RollingRaidBotSWSH(PokeBotState cfg, PokeTradeHub<PK8> hub) : PokeRoutineExecutor8SWSH(cfg), ICountBot
{
    private readonly RollingRaidSettings Settings = hub.Config.RollingRaidSWSH;
    ...
    private LobbyPlayerInfo[] LobbyPlayers = [new(), new(), new(), new()];
        ...
        try
        {
            ...
            if (Settings.RolloverPreventionTest)
            {
                ...
                if (await ReadDenData(true, token).ConfigureAwait(false))
                {
                    ...
                    if (await CheckIfDayRolled(true, token).ConfigureAwait(false))
                    {
    ...
    private async Task InnerLoop(long posix, CancellationToken token)
    {
        while (!token.IsCancellationRequested && Config.NextRoutineType == PokeRoutineType.RollingRaid)
        {
            ...
            if (!await AutoRollDen(code, posix, token).ConfigureAwait(false))
                return;
    ...
    private async Task<bool> AutoRollDen(int code, long posix, CancellationToken token)
    {
    ...
    private async Task<bool> CheckDen(CancellationToken token)
    {
        ...
        bool formLock = Settings.FormLock == string.Empty || formStr.Equals(Settings.FormLock, StringComparison.CurrentCultureIgnoreCase);
    ...
    private async Task<bool> ReadDenData(bool test, CancellationToken token)
    {
        ...
        try
        {
            ...
            {
                var eventOfs = DenUtil.GetEventDenOffset((int)Settings.ConsoleLanguage, RaidInfo.DenID, Settings.DenType, out _);
