...
#pragma warning restore CS8601 // Possible null reference assignment.
    ...
    private async Task RentalRoutine(ushort noteSpecies, CancellationToken token)
    {
        ...
        for (int i = 0; i < RentalOfsList.Length; i++)
        {
            ...
            {
                damage.Add(LairUtils.WeightedDamage([new()], pk, lairPk, MoveInfo, false).Max());
            }
        }
    ...
    {
        int[] movePP = [PlayerPk.Move1_PP, PlayerPk.Move2_PP, PlayerPk.Move3_PP, PlayerPk.Move4_PP];
    ...
    private bool CheckIfUpgrade(PK8[] party, PK8 lairPk)
    {
        ...
        var dmgWeightLair = LairUtils.WeightedDamage([new()], lairPk, LairBoss, MoveInfo, false);
    ...
    private async Task<ushort> SetHuntedPokemon(CancellationToken token)
    {
        if (HackyNoteCheck is -1 || LairBotUtil.DiscordQueueOverride)
        {
            for (int i = 0; i < 4; i++) // First note shifts due to yet unknown reasons, just clear possible slots, check which note to use on startup and after catching a legendary.
                await Connection.WriteBytesAsync([0], i is 0 ? LairSpeciesNote1 : i is 1 ? LairSpeciesNote2 : i is 2 ? LairSpeciesNote3 : LairSpeciesNote4, token);
            ...
            await Connection.WriteBytesAsync([0], firstNoteIsFirst ? LairSpeciesNote1 : LairSpeciesNote2, token).ConfigureAwait(false);
