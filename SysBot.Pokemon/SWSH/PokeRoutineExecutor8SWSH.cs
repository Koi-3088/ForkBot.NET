...
public abstract class PokeRoutineExecutor8SWSH(PokeBotState Config) : PokeRoutineExecutor<PK8>(Config)
{
    ...
    {
        var data = await Connection.ReadBytesAsync(GameVersion == GameVersion.SH ? InBattleRaidOffsetSH : InBattleRaidOffsetSW, 1, token).ConfigureAwait(false);
        return data[0] == (GameVersion == GameVersion.SH ? 0x40 : 0x41);
    }
