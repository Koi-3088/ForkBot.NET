...
{
    private readonly TradeCordSettings Settings = settings;
    ...
    public async Task<Results> ProcessTradeCord(TC_CommandContext ctx, string[] input)
    {
        ...
        try
        {
            ...
            {
                if (UserDict.TryGetValue(ctx.ID, out TCUser? value))
                {
                    user = value;
                }
                else
                ...
                {
                    if (UserDict.TryGetValue(ctx.GifteeID, out TCUser? giftRecipient))
                    {
                        giftee = giftRecipient;
                    }
                    else
                    {
    ...
    private Results CatchHandler(TCUser user, byte format = 8)
    {
        Results result = new();
        ...
        bool FuncCatch()
        {
            ...
            bool canGenerate = CanGenerateEgg(ref user, out IReadOnlyList<EvoCriteria> evos, out byte[] balls, out bool update);
            if (update)
            {
            ...
            if (Rng.CatchRNG >= 100 - Settings.CatchRate)
            {
                ...
                if (CherishOnly.Contains(Rng.SpeciesRNG) || Rng.CherishRNG >= 100 - Settings.CherishRate || mg != default)
                {
                    ...
                    if (mgRng != default)
                    {
                        ...
                        var info = new SimpleTrainerInfo { Gender = (byte)gender, Language = (int)language, OT = user.TrainerInfo.OTName, TID16 = user.TrainerInfo.TID16, SID16 = user.TrainerInfo.SID16, Context = Game is GameVersion.BDSP ? EntityContext.Gen8b : EntityContext.Gen8, Generation = format };
    ...
    private Results ListHandler(TCUser user, string input)
    {
        ...
        bool FuncList()
        {
            List<string> filters = input.Contains('=') ? [.. input.Split('=')] : [];
            if (filters.Count > 0)
            {
                filters.RemoveAt(0);
            ...
            bool isSpecies = TradeExtensions<T>.EnumParse<Species>(speciesAndForm ? input.Split('-')[0] : input) > 0;
            ...
            var strings = GameInfo.GetStrings(LanguageID.English.GetLanguageCode()).forms;
    ...
    private Results MassReleaseHandler(TCUser user, string input)
    {
        ...
        bool FuncMassRelease()
        {
            ...
            {
                catches = GetLookupAsClassObject<Dictionary<int, TCCatch>>(user.UserInfo.UserID, tableJoin, defaultSearch, true);
            }
            ...
            var specID = TradeExtensions<T>.EnumParse<Species>(speciesAndForm ? input.Split('-')[0] : input);
    ...
    private Results GiftHandler(TCUser user, TCUser m_user, string input)
    {
        ...
        bool FuncGift()
        {
            ...
            var specID = TradeExtensions<T>.EnumParse<Species>(match.Species);
    ...
    private Results NicknameHandler(TCUser user, string input)
    {
        ...
        bool FuncNickname()
        {
            ...
            bool clear = input.Equals("clear", StringComparison.CurrentCultureIgnoreCase);
            if (clear)
                pk.ClearNickname();
    ...
    private T EggProcess(TCDaycare dc, IReadOnlyList<EvoCriteria> evos, byte[] balls, byte generation, string[] trainerInfo, out string msg)
    {
        msg = string.Empty;
    ...
    private T SetProcessSWSH(string speciesName, string[] trainerInfo, byte eventForm)
    {
        ...
        if (Gigantamax.CanToggle(set.Species, set.Form) && Rng.GmaxRNG >= 100 - Settings.GmaxRate)
