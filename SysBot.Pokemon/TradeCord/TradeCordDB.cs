...
public abstract class TradeCordDatabase<T> : TradeCordBase<T> where T : PKM, new()
{
    protected T RngRoutineSWSH(T pkm, IBattleTemplate template, Shiny shiny)
    {
        ...
        else if (pkm is PK8 pk8 && enc is IOverworldCorrelation8 ow)
        {
            ...
            if (enc is EncounterStatic8 static8)
            {
                ...
                {
                    pk8.SetRandomIVs(Random.Next(static8.FlawlessIVCount, 7));
                }
            }
    ...
    protected T EggRngRoutine(IReadOnlyList<EvoCriteria> evos, byte[] balls, byte generation, string trainerInfo, Shiny shiny)
    {
        ...
        {
            formName = TradeExtensions<T>.FormOutput(speciesID, formID, out _);
        }
    ...
    protected bool EvolvePK(T pk, TimeOfDay tod, out string msg, out T? shedinja, AlcremieForms alcremie = AlcremieForms.None, RegionalFormArgument arg = RegionalFormArgument.None)
    {
        ...
        var sav = new SimpleTrainerInfo() { OT = pk.OriginalTrainerName, Gender = pk.OriginalTrainerGender, Generation = pk.Generation, Language = pk.Language, SID16 = pk.SID16, TID16 = pk.TID16, Context = Game is GameVersion.BDSP ? EntityContext.Gen8b : EntityContext.Gen8 };
    ...
    private static EvolutionTemplate EdgeCaseEvolutions(List<EvolutionTemplate> evoList, T pk, int alcremieForm, byte form, int item, TimeOfDay tod)
    {
        EvolutionTemplate? result = pk.Species switch
        {
            ...
            (ushort)Species.Cosmoem => pk.Version is GameVersion.UM or GameVersion.MN ? evoList.Find(x => x.EvolvesInto is (ushort)Species.Lunala) : evoList.Find(x => x.EvolvesInto is (ushort)Species.Solgaleo),
    ...
    protected bool CanGenerateEgg(ref TCUser user, out IReadOnlyList<EvoCriteria> criteria, out byte[] balls, out bool update)
    {
        update = false;
        criteria = [];
        balls = new byte[2];
    ...
    private static List<EvoCriteria> EggEvoCriteria(T pk1, T pk2)
    {
        ...
        for (int i = 0; i < list.Count; i++)
        {
            ...
            var tree = EvolutionTree.GetEvolutionTree(list[i].Context);
            var eggForm = tree.GetBaseSpeciesForm(list[i].Species, form);
            criteriaList.Add(new() { Species = eggForm.Species, Form = eggForm.Form });
        }
        return criteriaList;
    }
    ...
    protected MysteryGift? MysteryGiftRng(TradeCordSettings settings)
    {
        ...
        if (mg.Count > 0)
        {
            ...
            {
                mgRng = mg.ElementAt(Random.Next(mg.Count));
            }
        }
