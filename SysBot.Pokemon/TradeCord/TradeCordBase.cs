...
public abstract class TradeCordBase<T> where T : PKM, new()
{
    ...
    protected void ClearInactiveUsers()
    {
        ...
        {
            Base.EchoUtil.Echo("No inactive TradeCord users found to remove.");
        }
    }
        ...
        if (!exists)
        {
            ...
            for (int i = 0; i < dex.Count; i++)
            {
                ...
                for (int f = 0; f < forms.Length; f++)
                {
                    ...
                    bool gmax = Game == GameVersion.SWSH && Gigantamax.CanToggle(species, (byte)f);
    ...
    private static void EggBug()
    {
        ...
        while (reader.Read())
        {
            ...
            if ((pk.Nickname == nick || nickname == nick || nickname == "Egg") && !pk.IsEgg)
            {
                ...
                {
                    Base.LogUtil.LogError($"Catch {catch_id} (user {user_id}) is illegal.", "[SQLite]");
                }
            }
        }
        reader.Close();
                            ...
                            case CheckIdentifier.Ball:
                                {
                                    ...
                                    pk.Ball = (byte)balls[Random.Next(balls.Count)];
                                }; break;
