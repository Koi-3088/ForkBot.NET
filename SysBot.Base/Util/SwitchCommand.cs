...
public static class SwitchCommand
{
    ...
    /// </summary>
    /// <returns>Encoded command bytes</returns>
    public static byte[] Configure(SwitchConfigureParameter p, int ms, bool crlf = true)
        => Encode($"configure {p} {ms}", crlf);
    ...
    /// <param name="crlf">Line terminator (unused by USB protocol)</param>
    /// <returns>Encoded command bytes</returns>
    public static byte[] SetScreen(ScreenState state, bool crlf = true)
        => Encode($"screen{(state == ScreenState.On ? "On" : "Off")}", crlf);
