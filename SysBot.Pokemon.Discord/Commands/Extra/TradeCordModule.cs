...
public class TradeCordModule<T> : ModuleBase<SocketCommandContext> where T : PKM, new()
{
    ...
    public async Task EventVote()
    {
        ...
        var result = await Helper.ProcessTradeCord(ctx, []).ConfigureAwait(false);
    ...
    public async Task TradeCord()
    {
        ...
        if (Info.Hub.Config.TradeCord.TradeCordCooldown > 0)
        {
            ...
            else TradeCordHelper<T>.UserCommandTimestamps.Add(id, [DateTime.UtcNow]);
    ...
    public async Task TradeForTradeCord([Summary("Trade Code")] int code, [Summary("Numerical catch ID")] string id)
    {
        ...
        var result = await Helper.ProcessTradeCord(ctx, [id]).ConfigureAwait(false);
        if (!result.Success)
        {
    ...
    public async Task PokeList([Summary("Species name of a Pokémon")][Remainder] string content)
    {
        ...
        var result = await Helper.ProcessTradeCord(ctx, [content]).ConfigureAwait(false);
        if (!result.Success)
        {
    ...
    public async Task MassRelease([Remainder] string species = "")
    {
        ...
        var result = await Helper.ProcessTradeCord(ctx, [species]).ConfigureAwait(false);
        await Util.EmbedUtil(Context, name, result.Message).ConfigureAwait(false);
    }
    ...
    public async Task Daycare([Summary("Action to do (withdraw, deposit)")] string action, [Summary("Catch ID or elaborate action (\"All\" if withdrawing")] string id)
    {
        ...
        var result = await Helper.ProcessTradeCord(ctx, [action, id]).ConfigureAwait(false);
        if (!result.Success)
        {
            await Util.EmbedUtil(Context, name, result.Message).ConfigureAwait(false);
            return;
    ...
    public async Task TrainerInfoSet()
    {
        ...
        var result = await Helper.ProcessTradeCord(ctx, [ot, gender, tid, sid, lang]).ConfigureAwait(false);
        await Util.EmbedUtil(Context, name, result.Message).ConfigureAwait(false);
    }
    ...
    public async Task GiftItem([Remainder][Summary("Item name")] string input)
    {
        ...
        var result = await Helper.ProcessTradeCord(ctx, [input, count]).ConfigureAwait(false);
        await Util.EmbedUtil(Context, name, result.Message).ConfigureAwait(false);
    }
    ...
    private bool TradeCordParanoiaChecks(out string msg)
    {
        ...
        msg = string.Empty;
        List<int> rateCheck = [];
        IEnumerable<int> p = [Hub.Config.TradeCord.TradeCordCooldown, Hub.Config.TradeCord.CatchRate, Hub.Config.TradeCord.CherishRate, Hub.Config.TradeCord.EggRate, Hub.Config.TradeCord.ItemRate, Hub.Config.TradeCord.GmaxRate];
        ...
        IEnumerable<int> s = [Hub.Config.TradeCord.SquareShinyRate, Hub.Config.TradeCord.StarShinyRate];
